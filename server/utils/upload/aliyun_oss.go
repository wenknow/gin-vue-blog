...
func (*AliyunOSS) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	myFilename := fmt.Sprintf("%d-%s", timeStamp, file.Filename)
	myFilePath := global.CONFIG.AliyunOSS.BasePath + "/" + time.Now().Format("2006-01-02") + "/" + myFilename
	global.LOG.Info(myFilePath)
	// 上传文件流。
	err = bucket.PutObject(myFilePath, f)
	if err != nil {
		global.LOG.Error("function formUploader.Put() Failed", zap.Any("err", err.Error()))
		return "", "", errors.New("function formUploader.Put() Failed, err:" + err.Error())
	}

	return global.CONFIG.AliyunOSS.BucketUrl + "/" + myFilePath, myFilename, nil
}
